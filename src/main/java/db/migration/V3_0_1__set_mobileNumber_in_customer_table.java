...
public class V3_0_1__set_mobileNumber_in_customer_table implements JdbcMigration {
  private static Logger logger =
      LoggerFactory.getLogger(V3_0_1__set_mobileNumber_in_customer_table.class);
  ...
  public void migrate(Connection connection) throws Exception {
    try {
      ...
      logger
          .info("V3_0_1__set_mobileNumber_in_customer_table java migration completed successfully");
    } catch (Exception e) {
  ...
  void setMobileNumber(Connection connection) throws Exception {
    ...
    try {
      ...
      while (resultSet.next()) {
        ...
        preparedStatement =
            connection.prepareStatement("UPDATE customer SET mobileNumber = ? WHERE id = ?");
        ...
        if (isSuccess) {
          logger.debug(
              "setMobileNumber() :: given customer with id {} mobile number updated successfully",
              id);
          continue;
        }
        logger.debug(
            "setMobileNumber() :: given customer with id {} mobile number failed to update", id);
      }
    } finally {
